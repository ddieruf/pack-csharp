...
namespace Kpush.Sdks.Runner
{
	public class ProcessRunner
	{
		...
		private void SetEnvironmentVariable(ProcessStartInfo processStartInfo, string envVarName, List<string> envVarValues,
			char separator)
		{
		...
		private class ProcessState : IDisposable
		{
			...
			public void TryKill()
			{
				...
				{
					logger.LogDebug(
						$"Error while killing process '{_process.StartInfo.FileName} {_process.StartInfo.Arguments}': {ex.Message}");
#if DEBUG
